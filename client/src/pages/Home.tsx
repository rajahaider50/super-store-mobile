...
const STORAGE_KEY = "super-store-mobile:store:v3-zero";
...
export default function Home() {
  ...
  const deleteCategory = (id: string) => {
    ...
    if (hasProducts && !window.confirm(`This category has ${store.products.filter((product) => product.categoryId === id).length} product(s). Delete them too?`)) return;
    ...
    notify(hasProducts ? "Category and its products deleted" : "Category deleted");
  };
  ...
  const resetCatalog = () => { if (!window.confirm("Clear ALL products and categories? This resets to an empty catalog. Your orders will also be cleared.")) return; setStore({ ...seedState, categories: [], products: [] }); notify("Catalog cleared — start from scratch"); };
  ...
  const renderHome = () => <main className="public-main page-enter">
    ...
    <section className="home-section feature-section"><div className="section-heading"><div><SectionLabel index="02 / CATEGORIES">Shop by rhythm</SectionLabel><h2>Find your <em>everyday</em> better.</h2></div><button type="button" className="text-button" onClick={() => openAdmin()}><Plus size={15} /> Add category</button></div>{visibleCategories.length ? <div className="category-strip">{visibleCategories.slice(0, 4).map((category) => <button type="button" className="category-card" key={category.id} onClick={() => { setShopCategory(category.id); changeView("shop"); }}><img src={category.image} alt="" /><span className="category-card-overlay" /><div><small>{String(category.sort).padStart(2, "0")}</small><strong>{category.name}</strong><span>{category.description}</span></div><ChevronRight size={17} /></button>)}</div> : <EmptyState icon={<LayoutGrid size={24} />} title="No categories yet" detail="Add your first category to start building your store." action="Add category" onAction={openAdmin} />}</section>
    <section className="home-section"><div className="section-heading"><div><SectionLabel index="03 / THE EDIT">Selected now</SectionLabel><h2>Objects with <em>intent.</em></h2></div><button type="button" className="text-button" onClick={() => openAdmin()}><Plus size={15} /> Add product</button></div>{featuredProducts.length ? <div className="product-grid">{featuredProducts.slice(0, 4).map((product) => <ProductCard key={product.id} product={product} category={getCategory(product.categoryId)} currency={store.settings.currency} favorite={store.favorites.includes(product.id)} onOpen={() => setSelectedProduct(product)} onAdd={() => addToCart(product.id)} onFavorite={() => toggleFavorite(product.id)} />)}</div> : <EmptyState icon={<Box size={24} />} title="No products yet" detail="Add your first product to start selling." action="Add product" onAction={openAdmin} />}</section>
